...
<div class="page_title">管理中心 &gt; 数据管理 &gt;账目类型</div>
<div class="button_bar">
<input type="button" value="新增类型" onclick="to('accountType!detail.action?id=-1')"/>
</div>
<br/>
<table class="data_list_table">
	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach var="accountType" items="${pageResult.list }">
				<tr>
					<td class="list_data_number">${accountType.id }</td>
					<td class="list_data_text">${accountType.name }</td>
					<td class="list_data_text">
					<fmt:formatDate value="${accountType.createTime }" pattern="yyyy-MM-dd" var="ct"/>
						${ct }
					</td>
					<td class="list_data_text">
						<input type="button" onclick="to('accountType!detail.action?id=${accountType.id }')" value="编辑">
						<input type="button" onclick="del('accountType!delete.action?id=${accountType.id }')" value="删除">
					</td>
				</tr>
			</c:forEach>
		</c:otherwise>
...
function mysearch(p){
	...
	to('accountType!list.action?p='+p);
}
</script>
</body>
</html>