...
<script type="text/javascript">
	function mysearch(p){
			...
			myform.action="account!list.action?p="+p;
			myform.submit();
		}
</script>
</head>
<body>
<div class="page_title">管理中心 &gt; 数据管理 &gt;账目列表</div>
...
<select name="atId">
	...
	<c:forEach items="${accountTypeList}" var="accountType">
		<option value="${accountType.id }" <c:if test="${accountType.id==atId }">selected</c:if>>${accountType.name }</option>
	</c:forEach>
</select>
创建日期
...
<input type="button" value="记账" onclick="to('account!detail.action?id=-1')"/>
</div>
<br/>
...
<table class="data_list_table">
	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach var="account" items="${pageResult.list }">
				<tr>
					<td class="list_data_number">${account.id }</td>
					<td class="list_data_text">${account.accType.name }</td>
					<td class="list_data_text">${account.price }</td>
					<c:set value="${account.price+allMoney }" var="allMoney"></c:set>
					<td class="list_data_text">${fn:substring(account.explanation,0,10 )}</td>
					<td class="list_data_text">
					<fmt:formatDate value="${account.createTime }" pattern="yyyy-MM-dd" var="ct"/>
						${ct }
					</td>
					<td class="list_data_text">
						<input type="button" onclick="to('account!detail.action?id=${account.id }')" value="编辑">
						<input type="button" onclick="del('account!delete.action?id=${account.id }')" value="删除">
					</td>
				</tr>
			</c:forEach>
		</c:otherwise>
...
<div class="pager">
总计:<span class="red_test"> ${allMoney } </span>  元&nbsp;&nbsp;&nbsp;	<c:set var="page" value="${pageResult}"></c:set> 共${page.recTotal }条记录 每页<input
