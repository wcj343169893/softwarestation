...
<c:choose>
	...
	<c:otherwise>
		<c:forEach items="${pageResult.list}" var="si" varStatus="vs" > 
			<c:out value="${vs.count }"></c:out>.<c:if test="${si[1].recommend==1}"><img src="img/commend.gif" alt="推荐" /></c:if><a href="showsoftwareInfo.php?id=${si[1].id}&amp;mid=${mid }">[${si[1].softwareType.name }]${si[1].name }</a><br/>
		</c:forEach>
