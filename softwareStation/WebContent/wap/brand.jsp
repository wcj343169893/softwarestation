...
<p>
	...
	<a href="setmodel.php?bid=1">诺基亚全部机型</a><br />
	【其他品牌】<br/>
	<c:set value="${fn:length(brandDTO.phoneBrandList)}" var="maxsize"></c:set>
	<c:forEach items="${brandDTO.phoneBrandList}" var="brand" varStatus="vs" begin="1"><a href="setmodel.php?bid=${brand.id}">${brand.name}</a><c:choose><c:when test="${vs.count % 3 == 0 ||maxsize==vs.count}"><br/></c:when><c:otherwise>|</c:otherwise></c:choose></c:forEach>
	<br />
	【温馨提示】<br/>
